...
func _init_state_machine() -> void:
	...
	# hsm.add_transition(move_state, air_state, move_state.EVENT_JUMP) # Перехід із стану руху в повітряний
	# hsm.add_transition(idle_state, air_state, idle_state.EVENT_JUMP) # Перехід із стану очікування в повітряний
...
func _process(delta: float) -> void:
	...
	elif Input.get_axis("move_left", "move_right") != 0 and hsm.get_active_state() != air_state:
